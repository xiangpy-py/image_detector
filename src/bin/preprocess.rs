...
struct Args {
    #[arg(long, default_value = "/root/autodl-tmp/datasets/paultimothymooney/chest-xray-pneumonia/versions/2/chest_xray/chest_xray")]
    root: PathBuf,
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (split_dir, split_name) in &splits {
        ...
        for (i, (path, label)) in image_paths.iter().enumerate() {
            let img = load_and_preprocess(path, args.size)?;
...
fn load_and_preprocess(path: &Path, size: u32) -> Result<Array3<f32>, ImageError> {
    ...
    for (x, y, pixel) in img.enumerate_pixels() {
        for c in 0..3 {
            ...
            tensor[[c, y as usize, x as usize]] = (val - IMAGENET_MEAN[c]) / IMAGENET_STD[c];
        }
    }
