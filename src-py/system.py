...
def get_active_dataset() -> str | None:
    """获取当前默认数据集名称。"""
    return load_dataset_registry().get("active")
...
def get_default_dataset_root() -> Path:
    ...
    if env_roots:
        first_path = next((p.strip() for p in env_roots.split(",") if p.strip()), None)
        if first_path:
    ...
    candidates = [
        ...
        Path.home() / "datasets" / "chest-xray-pneumonia" / "chest_xray",
...
def get_multiprocessing_start_method() -> str:
    """统一使用 spawn，避免 fork 模式下 CUDA context 死锁。
    """
    return "spawn"
